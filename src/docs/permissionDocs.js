 ...
 *         **Responses for `user_type` SUPERADMIN replace the mask:** `has_access` is true for every row and
 ...
 *         has_access:
 *           type: boolean
 *           description: Whether this permission is granted to the org/unit context (after tier hidden rules). Not used as sole source for Super Admin (see SUPERADMIN override).
 ...
 *       **Super Admin** (`SUPERADMIN`) may pass `org_id` query to preview another tenant's pool; response rows use the Super Admin mask (all `has_access` true, locks off).
 *     tags: [Permissions]
 *     security:
 *       - bearerAuth: []
 *     parameters:
